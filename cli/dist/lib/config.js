...
export async function loadConfig() {
    ...
    try {
        ...
        if (nodeError.code === 'ENOENT') {
            throw new ConfigError('Configuración no encontrada. Ejecuta `widot init` primero.');
        }
        if (nodeError.code === 'EACCES') {
            throw new ConfigError(`Sin permiso de lectura en ${CONFIG_PATH}. Verifica permisos del archivo.`);
        }
        throw new ConfigError(`No se pudo leer ${CONFIG_PATH}: ${nodeError.message}`);
    }
    let parsed;
    try {
