...
export async function runPull(): Promise<void> {
  ...
  for (const remote of remoteFiles) {
    ...
    try {
      ...
      const plaintext = decryptDotfile(
        download.encrypted_payload,
        config.passphrase,
      );
      ...
      if (decryptedHash !== remoteHash) {
        throw new Error(
          'content_hash no coincide tras descifrar: passphrase incorrecta o datos corruptos.',
        );
      }
...
function logPullError(originalPath: string, error: unknown): void {
